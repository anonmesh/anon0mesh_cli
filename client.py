...
# ── Arcium client-side helpers (optional) ─────────────────────────────────────
try:
    from arcium_client import rescue_keygen, rescue_encrypt, rescue_decrypt, rescue_shared_secret
    HAS_ARCIUM = True
except ImportError:
    HAS_ARCIUM = False
...
class BeaconAnnounceHandler:
    ...
    def received_announce(self, destination_hash, announced_identity, app_data):
        ...
        # Debug: log every announce we receive so we can see what's arriving
        # log_info(f"Announce received  hash={short}  "
        #          f"app_data={app_data!r}  identity={'yes' if announced_identity else 'no'}")

        # Normalise app_data — RNS may deliver it as bytes or None
        if app_data is None:
            return
        # Compare both raw bytes and decoded string variants
        if app_data != ANNOUNCE_DATA:
            try:
                if app_data.decode("utf-8", errors="replace") != ANNOUNCE_DATA.decode("utf-8"):
                    # log_info(f"Announce filtered  expected={ANNOUNCE_DATA!r}  got={app_data!r}")
                    return
            except Exception:
                return
...
def confidential_get_balance(address: str) -> None:
    ...
    try:
        ...
        log_err(f"Invalid address: {exc}"); return

    # Encrypt address with MXE public key (X25519 + RescueCipher)
    log_info(f"Encrypting address for Arcium MPC...")
    try:
        enc = rescue_encrypt(mxe_pubkey_hex, address_bytes)
    except Exception as exc:
        log_err(f"Encryption failed: {exc}"); return

    # Send encrypted request to beacon
    resp = rpc_call("getBalance", [{
        "enc_address":    enc["ciphertexts"][0],   # [u8;32]
    ...
    if resp.get("status") == "error":
        log_err(f"Arcium error: {resp.get('message', '?')}"); return

    r = resp.get("result", resp)
    if r.get("status") == "error":
        log_err(f"Arcium error: {r.get('message', '?')}"); return

    # Decrypt result locally — beacon never saw the balance
    try:
        shared_secret = rescue_shared_secret(enc["shared_secret_hex"] if "shared_secret_hex" in enc
                                             else enc.get("privkey_hex", ""),
...
def get_slot():
    ...
    if resp is None:
        log_err("No response from beacon"); return
    if "error" in resp:
...
def get_recent_blockhash():
    ...
    r = resp.get("result")
    # getLatestBlockhash always returns {"context":..., "value":{"blockhash":...}}
...
{BOLD}RPC commands:{RESET}
  balance  <ADDRESS>                Fetch SOL balance (plain)
  cbalance <ADDRESS>                Fetch SOL balance (confidential via Arcium MPC)
  slot                              Current slot
  height                            Block height
...
def repl():
    ...
    while True:
        ...
        if cmd in ("quit", "exit", "q"):
            ...
            get_balance(parts[1])
        elif cmd == "cbalance" and len(parts) >= 2:
            confidential_get_balance(parts[1])
        elif cmd == "slot":
            get_slot()
        elif cmd == "height":
            get_block_height()
...
def main():
    ...
    if args.discover and not args.beacon:
        log_info("Waiting for beacon announces (60 s, beacons burst-announce every 15 s)...")
